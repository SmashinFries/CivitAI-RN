...
export const ImageCard = ({ maxHeight, width, item, isSaved }: ImageCardProps) => {
    ...
    return (
        <Pressable
            onPress={() => router.push({ pathname: 'image/[id]', params: { id: item?.id } })}
        ...
        >
            ...
            <HasMetaDataTag hasMeta={item.meta ? true : false} isSaved={isSaved} />
        </Pressable>
    );
};
...
export const ModelImageCard = ({ index, maxHeight, width, item }: ModelImageCardProps) => {
    ...
    return (
        <Pressable onPress={() => setIsBlur((prev) => !prev)} style={{ borderRadius: 12 }}>
            <Image
                source={{ uri: item?.url }}
                blurRadius={item.nsfw !== CivitAiNSFW.None && isBlur ? 200 : 0}
                transition={{ effect: 'cross-dissolve', duration: 800 }}
                contentFit="cover"
                style={{
                    aspectRatio: item?.width / item?.height,
                    borderRadius: 12,
