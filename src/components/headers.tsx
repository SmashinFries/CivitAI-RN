...
export const ImagesHeader = ({ navigation, options, route, back, sort, updateSort }: NativeStackHeaderProps & { sort:CivitAiImageSort, updateSort:(newSort:CivitAiImageSort) => void }) => {
    const title = getHeaderTitle(options, route.name);
    ...
    return (
        <Appbar.Header>
            ...
            <Menu
                ...
                {menuOptions.map((item, index) => <Menu.Item key={index} leadingIcon={item === sort ? 'check' : undefined} onPress={() => onMenuItemPress(item)} title={item} />)}
            </Menu>
